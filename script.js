...
function exportData() {
    ...
    a.href = url;
    a.download = `jt-school-timer-data-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
...
function saveData() {
    ...
    localStorage.setItem('jtSchoolTimerData', JSON.stringify(data));
}

// 데이터 로드
function loadData() {
    const savedData = localStorage.getItem('jtSchoolTimerData');
    if (savedData) {
        try {
